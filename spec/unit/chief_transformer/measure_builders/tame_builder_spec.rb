...
describe ChiefTransformer::MeasureBuilder::TameBuilder do
  let(:mfcm)           { create :mfcm }
  let(:mfcm_with_tame) { create :mfcm, :with_tame }

  subject { ChiefTransformer::MeasureBuilder::TameBuilder }

  describe '.build' do
    it 'builds CandidateMeasures based on present Chief Tame records' do
    end
